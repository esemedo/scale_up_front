...
export const TypeList: React.FC<TypeListProps> = ({
  ...
  const handleReturnSelectElement = (
    element?: number,
    typeElement?: string,
  ) => {
  ...
  return (
    <Tabs defaultValue="category" className="w-[400px]">
      <TabsList className="grid w-full grid-cols-3">
        <TabsTrigger
          value="subject"
          onClick={() => handleTabsTriggerClick("subject")}
        >
          Matière/Module
        </TabsTrigger>
        <TabsTrigger
          value="category"
          onClick={() => handleTabsTriggerClick("category")}
        >
          Catégorie
        </TabsTrigger>
        <TabsTrigger
          value="place"
          onClick={() => handleTabsTriggerClick("place")}
        >{`Lieu d'enseignement`}</TabsTrigger>
      </TabsList>
