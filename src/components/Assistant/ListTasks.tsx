...
import {  PRIORITY_COLOR,STATUS_COLOR_BG, STATUS_COLOR_TEXT } from '../../lib/constants';
...
function ListTasks({data, setData ,handleItemClick, selectedItem, status =0, session, updateState} :ListTasksProps) {
  ...
  React.useEffect(() => {
      ...
      axios
        ...
        .catch((error) => {
          toast({
            ...
            "Une erreur s'est produite lors de la récupération des tâches.": "Une erreur inattendue s'est produite.",
          })
        });
  }
    }, [session, updateState, priority]);
    ...
    return (    
      <div>
        <div className='mt-8'>
        <SelectPriority priority={priority} handleChangePriority={handleChangePriority}  />
        </div>
      <ul className="task-list overflow-auto mt-6 flex flex-col items-center">
          {data.map((item: TasksProps) => {
            return <li
              ...
              className={`bg-${STATUS_COLOR_BG[item.status]} text-${STATUS_COLOR_TEXT[item.status]}  p-5 mt-3 rounded-lg relative w-11/12 ${selectedItem?.id === item.id ? "selected border-2 border-electric-blue border-solid" : ""}`}
